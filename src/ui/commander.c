...
static void draw_remote_panel(struct AppState *app, WINDOW *win) {
  ...
  mvwprintw(win, 2, 2, "Path: %s", app->remote_path);
  mvwprintw(win, 4, 2, "Directory listing needs TSR support.");
  mvwprintw(win, 5, 2, "Use d to download a typed DOS filename.");
  mvwprintw(win, 6, 2, "Use u from the local pane to upload.");

  if (rows > 9 && cols > 22) {
    wattron(win, A_DIM);
    mvwprintw(win, rows - 3, 2, "Planned: V1_DIR_LIST_* + DOS path ops");
    wattroff(win, A_DIM);
  }
}
...
static void upload_selected(struct AppState *app) {
  ...
  char remote_name[FILE_TRANSFER_NAME_BYTES];
  int rc;
  ...
  if (strlen(entry->name) >= sizeof(remote_name)) {
    set_status(app, "Remote filename must fit in %d characters.",
...
static void download_prompted(struct AppState *app) {
  ...
  def_prog_mode();
  endwin();
  rc = file_transfer_get(&app->sock, app->active_host->if_addr, remote_name,
                         local_path);
  shell_transfer_pause();
  reset_prog_mode();
  ui_reset();
  ...
  switch (ch) {
    ...
    case KEY_ENTER:
      if (app->focus == FOCUS_LOCAL) {
        ...
        set_status(app, "Remote browsing waits on V1_DIR_LIST_* TSR support.");
      }
      break;
    case 'u':
    case 'U':
    ...
    case 'r':
    case 'R':
      if (local_panel_load(local, local->cwd)) {
...
static void run_commander(struct AppState *app) {
  ...
  set_status(app, "Connected. Remote listing is waiting on TSR protocol work.");
